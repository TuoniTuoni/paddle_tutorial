...
class RandomScale(object):
    ...
    def __call__(self,image,label=None):
        if self.step==0:
            self.random_scale=np.random.uniform(self.min_scale,self.max_sacle,1)[0]
        else:
...
def main():
    ...
    #img=cv2.imread('./dummy_data/JPEGImages/2008_000064.jpg')
    # TODO: crop_size
    # TODO: Transform: RandomSacle, RandomFlip, Pad, RandomCrop
    crop_size=256
    augment=Compose([RandomScale(),
                    RandomFlip(),
    ...
    for i in range(10):
        new_image,new_label=augment(image,label)
        cv2.imwrite('%d.jpg'%i,new_image)
        cv2.imwrite('%d.png'%i,new_label)
    #augment=Compose([Resize(size=300)])


    #new_image_1,new_label_1=augment(image,label)
    #cv2.imwrite('tmp_new_1.jpg',new_image_1)
    #cv2.imwrite('tmp_new_1.png',new_label_1)
